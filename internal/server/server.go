...
func (s *Server) handleCookies(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut:
		...
		status := cookieStatus(cfg.App.CookiesFile)
		s.log.Infof("cookies file updated path=%s size=%d", status.Path, status.Size)
...
func writeCookieFile(path, content string) error {
	...
	normalized, err := normalizeCookieContent(content)
	if err != nil {
		return err
	}
...
func normalizeCookieContent(content string) (string, error) {
	...
	header := rawCookieHeader(content)
	var lines []string
	for _, part := range strings.Split(header, ";") {
		...
		lines = append(lines, ".douyin.com\tTRUE\t/\tTRUE\t2147483647\t"+name+"\t"+value)
	}
	if len(lines) == 0 {
		return "", errors.New("cookie content is not Netscape format and no raw cookie pairs were found")
	}

	out := "# Netscape HTTP Cookie File\n"
	out += "# Generated by douyin-nas-monitor from a raw Cookie header.\n"
